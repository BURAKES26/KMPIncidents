...
import com.example.kmpincidents.util.ImageUrlHelper
import com.example.kmpincidents.util.IncidentDisplayHelper
import com.example.kmpincidents.util.IncidentDisplayHelper.getCategoryLabel
import com.example.kmpincidents.util.IncidentDisplayHelper.formatDateForDisplay
import com.example.kmpincidents.util.IncidentDisplayHelper.getStatusColor
import com.example.kmpincidents.viewmodel.MyIncidentDetailViewModel
...
fun MyIncidentDetailScreen(
    ...
    val successUpdateMessage = stringResource(Res.string.incident_updated_successfully)
    val failureUpdateMessage = stringResource(Res.string.failed_to_update_incident)

    LaunchedEffect(uiState.updateResult) {
        uiState.updateResult?.let { result ->
            val message = if (result is ApiResult.Success) {
    ...
    val successDeleteMessage = stringResource(Res.string.incident_deleted_successfully)
    val failureDeleteMessage = stringResource(Res.string.failed_to_delete_incident)

    LaunchedEffect(uiState.deleteResult) {
        uiState.deleteResult?.let { result ->
            if (result is ApiResult.Success) {
                scope.launch { snackbarHostState.showSnackbar(successDeleteMessage) }
                onNavigateBack()
            } else {
                val message = "$failureDeleteMessage ${
                    when (result) {
                        is ApiResult.HttpError -> result.message
                        is ApiResult.NetworkError -> result.exception.message
                        is ApiResult.Unauthorized -> { onNavigateBack(); "" }
    ...
    DisposableEffect(Unit) { onDispose { viewModel.clearSelectedIncident() } }
    ...
    if (showResolvedDialog) {
        AlertDialog(
            ...
            title = {
                Text(
                    text = stringResource(Res.string.incident_already_resolved),
            ...
            text = {
                Text(
                    text = stringResource(Res.string.this_incident_has_been_marked_as_resolved_and_can_no_longer_be_modified),
                    fontSize = 15.sp,
                    lineHeight = 22.sp
                )
            },
    ...
    if (showCannotDeleteDialog) {
        AlertDialog(
            ...
            title = {
                Text(
                    text = stringResource(Res.string.cannot_delete_incident),
                    fontWeight = FontWeight.Bold,
                    fontSize = 20.sp
                )
            },
            text = {
                Text(
                    text = stringResource(Res.string.incidents_that_are_assigned_or_resolved_cannot_be_deleted),
                    fontSize = 15.sp,
                    lineHeight = 22.sp
                )
            },
    ...
    if (showDeleteConfirmDialog) {
        AlertDialog(
            ...
            title = {
                Text(
                    text = stringResource(Res.string.delete_incident),
                    fontWeight = FontWeight.Bold,
                    fontSize = 20.sp
                )
            },
            text = {
                Text(
                    text = stringResource(Res.string.are_you_sure_you_want_to_delete_this_incident_this_action_cannot_be_undone),
                    fontSize = 15.sp,
                    lineHeight = 22.sp
                )
            },
    ...
    Scaffold(
        ...
        topBar = {
            TopNavBar(
                title = stringResource(Res.string.incident_details),
                showBackButton = true,
        ...
        Box(
            ...
            if (currentIncident == null) {
                Box(
                    ...
                    Text(
                        text = stringResource(Res.string.incident_details_not_available),
...
private fun ActionButtons(
    ...
    Row(
        ...
        OutlinedButton(
            ...
            Icon(
                imageVector = DeleteIcon,
                contentDescription = stringResource(Res.string.delete_incident),
            ...
            Row(
                ...
                Column(
                    ...
                    Row(
                        ...
                        Text(
                            text = if (incident.completedAt != null) stringResource(Res.string.completed) else stringResource(
                                Res.string.pending
                            ),
                            fontSize = 10.sp,
                            fontWeight = FontWeight.Bold,
        ...
        Column(
            ...
            if (incident.images.isNotEmpty()) {
                    ...
                    items(incident.images.count()) { index ->
                        ...
                        Surface(
                            ...
                            if (!imageUrl.isNullOrBlank()) {
                                AsyncImage(
                                    model = imageUrl,
                                    contentDescription = stringResource(Res.string.incident_image) + " ${index + 1}",
                                    modifier = Modifier.fillMaxSize(),
                ...
                Box(
                    ...
                    Text(
                        text = stringResource(Res.string.no_photos_available),
                        fontSize = 14.sp,
                        color = Color(0xFF9CA3AF),
                        fontWeight = FontWeight.Medium
                    )
...
private fun IncidentLocationCard(
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = stringResource(Res.string.tap_on_the_map_to_select_a_new_location),
                fontSize = 12.sp,
                color = Color(0xFF6B7280),
